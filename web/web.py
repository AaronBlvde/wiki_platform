...
def login():
    ...
    if request.method == "POST":
        ...
        if resp.status_code == 200:
            ...
            session["login"] = username  # сохраняем логин
...
def dashboard():
    ...
    try:
        resp = requests.get(f"{WIKI_URL}/pages", headers={"Authorization": f"Bearer {token}"}, timeout=5)
...
def create_page():
    ...
    try:
        ...
        if resp.status_code != 200:
            print("Create page error:", resp.text)
...
def delete_page(page_id):
    ...
    if not token:
        return redirect(url_for("login"))
    try:
        resp = requests.delete(f"{WIKI_URL}/pages/{page_id}", headers={"Authorization": f"Bearer {token}"}, timeout=5)
        if resp.status_code != 200:
            print("Delete page error:", resp.text)
    except Exception as e:
    ...
    if request.method == "POST":
        ...
        if title.strip():
            try:
                ...
                if resp.status_code != 200:
                    print("Update page error:", resp.text)
            except Exception as e:
                print("Error updating page:", e)
        return redirect(url_for("dashboard"))

    try:
        resp = requests.get(f"{WIKI_URL}/pages/{page_id}", headers={"Authorization": f"Bearer {token}"}, timeout=5)
